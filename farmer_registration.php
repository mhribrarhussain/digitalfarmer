...
if(isset($_POST['register'])) {
    ...
    $check = mysqli_query($conn, "SELECT * FROM farmers WHERE email='$email'");
    if(mysqli_num_rows($check) > 0){
        $error = "Email already registered!";
    } else {
        $insert = mysqli_query($conn, "INSERT INTO farmers (fullname,email,phone,password) VALUES ('$fullname','$email','$phone','$password')");
        if($insert){
...
<h1 class="text-[#111811] dark:text-white tracking-light text-[32px] font-bold leading-tight text-left pb-3 pt-6">Create Your Farm Account</h1>
<form class="flex flex-col gap-4" action="farmer_registration.php" method="POST">
<?php if(!empty($error)): ?>
...
<p class="text-center text-sm text-gray-600 dark:text-gray-400 pt-4">Already have an account? <a class="font-medium text-primary hover:underline" href="farmer_login.php">Login</a></p>
</div>
</div>
</div>
</div>
